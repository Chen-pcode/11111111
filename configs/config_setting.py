...
class setting_config:
    ...
    network = 'egeunet'
    @staticmethod
    def make_transforms(dataset, height, width):
        # Rebuild after selecting the dataset and seeding; retain the original operations.
        train = transforms.Compose([
            myNormalize(dataset, train=True), myToTensor(),
            myRandomHorizontalFlip(p=0.5), myRandomVerticalFlip(p=0.5),
            myRandomRotation(p=0.5, degree=[0, 360]), myResize(height, width)
        ])
        val = transforms.Compose([
            myNormalize(dataset, train=False), myToTensor(), myResize(height, width)
        ])
        return train, val
    # 模型结构配置：
    ...
    model_config = {
        ...
        'bridge': True,
        'gt_ds': True,
         'freq_mode': 'off',  #可选集合：`{'off', 'low', 'high', 'both', 'spatial'}`  
        'freq_gate': 'mask', #可选集合：`{'none', 'feature', 'mask'}`
        'freq_stages': (1, 2, 3),  #GAB1、GAB2、GAB3、GAB4、GAB5，一共 5 个 GAB 桥）
    }
